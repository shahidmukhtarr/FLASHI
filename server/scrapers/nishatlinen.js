...
const STORE_NAME = 'Nishat Linen';
const STORE_URL = 'https://nishatlinen.com';
const STORE_COLOR = '#2c5f8a'; // Nishat blue

// Collection pages — covers men, women, accessories, and current sales
const COLLECTION_URLS = [
  'https://nishatlinen.com/collections/women',
  'https://nishatlinen.com/collections/men',
  'https://nishatlinen.com/collections/accessories',
  'https://nishatlinen.com/collections/sale26',
];
...
// Fashion keywords: when searching "shirt", "suit", "lawn" etc. Nishat products
// often have names like "3 Piece - Printed Suit" that don't contain the word "shirt"
// but ARE relevant. We accept any product from Nishat for fashion queries.
const FASHION_QUERY_WORDS = new Set([
  'shirt', 'kurta', 'kurti', 'kameez', 'suit', 'lawn', 'pret', 'stitched',
  'unstitched', 'fabric', 'dupatta', 'shalwar', 'trouser', 'palazzo', 'chiffon',
  'silk', 'cotton', 'embroidered', 'printed', 'bridal', 'formal', 'casual',
  'women', 'ladies', 'men', 'gents', 'boys', 'girls', 'piece', '2piece', '3piece',
  'dress', 'top', 'blouse', 'jacket', 'coat', 'scarf', 'shawl', 'accessories',
  'clothing', 'clothes', 'fashion', 'wear', 'outfit', 'collection', 'seasonal',
  'summer', 'winter', 'nishat',
]);

function isFashionQuery(query) {
  const words = query.toLowerCase().split(/\s+/);
  return words.some(w => FASHION_QUERY_WORDS.has(w));
}
...
async function fetchFromCollection(collectionUrl, query, limit) {
  try {
    ...
    // For fashion queries, accept all products from this store (Nishat product names
    // don't always contain the search word but are still relevant)
    const acceptAll = isFashionQuery(query);
    const queryWords = query.toLowerCase().split(/\s+/)
      .filter(w => w.length > 1 && !['nishat', 'linen', 'nishatlinen'].includes(w));
    ...
    return items
      ...
      .filter(p => {
        if (!p) return false;
        if (acceptAll || queryWords.length === 0) return true;
...
export async function searchProducts(query, limit = 50) {
  ...
  try {
    ...
    $('.grid__item .card-wrapper, .product-item, li.grid__item, .product-card').each((_, el) => {
