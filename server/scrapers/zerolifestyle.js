...
const STORE_NAME = 'Zero Lifestyle';
const STORE_URL = 'https://zerolifestyle.co';
const STORE_COLOR = '#00c853';

// Collection pages for browsing/fallback
const COLLECTION_URLS = [
  'https://zerolifestyle.co/pages/smart-watches',
  'https://zerolifestyle.co/pages/earbuds-zbuds',
  'https://zerolifestyle.co/collections/headphones',
];

function parseShopifyProduct(item) {
  const price = parsePrice(item.price || item.price_min || '0');
  if (!price) return null;
  const comparePrice = parsePrice(item.compare_at_price || item.compare_at_price_min || item.compare_at_price_max || '0');
...
async function fetchCollectionProducts(collectionUrl, query, limit = 20) {
  try {
    ...
    if (type === 'collections') {
      ...
      const queryWords = query.toLowerCase().split(/\s+/).filter(w => w.length > 1 && w !== 'zero' && w !== 'lifestyle');
      return items
        .map(item => {
...
export async function searchProducts(query, limit = 50) {
  ...
  try {
    ...
    console.log(`[${STORE_NAME}] Found ${products.length} products for "${query}" (HTML fallback)`);
    return products;
  } catch (e) {
